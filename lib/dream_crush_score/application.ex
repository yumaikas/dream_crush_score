defmodule DreamCrushScore.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      # Start the Ecto repository
      DreamCrushScore.Repo,
      # Start the Telemetry supervisor
      DreamCrushScoreWeb.Telemetry,
      # Start the PubSub system
      {Phoenix.PubSub, name: DreamCrushScore.PubSub},
      # Start the Endpoint (http/https)
      DreamCrushScoreWeb.Endpoint,
      DreamCrushScore.Rooms,
      # Start a worker by calling: DreamCrushScore.Worker.start_link(arg)
      # {DreamCrushScore.Worker, arg}
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: DreamCrushScore.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    DreamCrushScoreWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
